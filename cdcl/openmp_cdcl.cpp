...
private:
  ...
  int pick_branching_variable_parallel(); // to pick the next free assignment
...
int SATSolverCDCL::CDCL() {
  ...
  while (!all_variables_assigned()) {
    ...
    int picked_variable = pick_branching_variable_parallel(); // pick the next free
...
int SATSolverCDCL::CDCL_parallel() {
  int decision_level = 0;  // initial decision level
  ...
  while (!all_variables_assigned()) {
    int picked_variable = -1;

    #pragma omp parallel shared(picked_variable)
    {
      #pragma omp single
      picked_variable = pick_branching_variable_parallel();
    ...
    int conflict_occurred = 0;

    while (!conflict_occurred) {
 ...
 */
int SATSolverCDCL::pick_branching_variable_parallel() {
  int random_value = 1; // Initialize random_value for OpenMP usage
  int variable = 0; // Initialize variable for assignment

  #pragma omp parallel private(random_value, variable)
  {
    #pragma omp single
    {
      // Generate a random integer between 1 and 10 for deciding the mechanism of choosing
      random_value = std::uniform_int_distribution<int>(1, 10)(generator);
    }

    // Check if the thread is executing parallel sections based on the random_value
    if (random_value > 4 || assigned_literal_count < literal_count / 2) {
      // Execute section that finds the variable with the highest frequency out of those unassigned
      // This section is executed by one thread only
      #pragma omp single
      {
        pick_counter++; // Increment the number of picks so far this way
              
        // Perform frequency adjustments after a certain number of picks
      ...
      // Execute section that picks the variable randomly
      // This section is executed by multiple threads trying to find an unassigned variable randomly
      int attempt_counter = 0;
      bool too_many_attempts = false;

      while (!too_many_attempts && attempt_counter < 10 * literal_count) {
        #pragma omp critical
        {
          variable = std::uniform_int_distribution<int>(0, literal_count - 1)(generator);
        }
        ...
        if (literal_frequency[variable] != -1) {
          if (literal_polarity[variable] >= 0) {
            ...
            break; // Break out of the loop once an unassigned variable is found
        }
...
void SATSolverCDCL::solve_parallel() {
  int result_status = CDCL_parallel();
  show_result(result_status);
}
